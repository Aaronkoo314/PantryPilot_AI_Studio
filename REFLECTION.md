...
prompt defined them, and that definition was not mine. It came from losing an argument in ChatGPT:
I asked for "all people, from beginners to professional chefs" and was told that a product for
everyone is a category, not a user. The narrowed wording — *non-professional home cooks, including
beginners, students, busy workers, parents, and experienced everyday cooks* — went into the prompt
verbatim and is therefore in both repositories. The full exchange is in
[§1 of the companion log](https://github.com/Aaronkoo314/pantrypilot/blob/main/PROMPTS.md#round-2--the-user-definition-where-i-was-wrong).
...
proposed and substituting my own; the nine features in the paragraph I wrote; losing the argument
about the audience; and the guardrail that exists because a feature I asked for was identified as a
scope failure before it was built. All of that is documented in
[§1 of the companion log](https://github.com/Aaronkoo314/pantrypilot/blob/main/PROMPTS.md#1-where-the-master-prompt-came-from),
and all of it is genuinely mine or genuinely a decision I made about someone else's suggestion.
...
meals. AI Studio implemented Fitness as a filter: selecting it **removes** meals from the list
rather than ranking them upward, and `sortBy: 'match'` is untouched by preference.

One word, two readings, and the feature changed nature — a score became a boolean, and three stated
criteria collapsed into one flag with an unwritten threshold (§Q2.3). The companion build read the
same word as a ranking and derived it from the dataset's own nutrition signal.

I never noticed the ambiguity when I pasted the prompt. It reads unambiguous in English and is not.
